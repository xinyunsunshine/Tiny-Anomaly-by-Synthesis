...
fig_save_dir = '/home/sunsh16e/diffunc/baseline/data/compare'
upsample_model = 'dinov2'
cond_type = 'l2_p'
unc_type = 'SAM_HighRes_cos'

all_folders = ['trail-13', 'park-8', 'trail-5', 'trail-7'] #['trail-13', 'park-8', 'trail-5', 'trail-7', 'creek', 'park-1']
for folder in all_folders:
    ...
    for rel_ood_img_path in image_path_name:
        ...
        ##### gmm on generated
        unc_save_path = os.path.join(gen_save_path_gmm, upsample_model, f'{subname}.pt')
        unc_ts = torch.load(unc_save_path)
        unc_ts = F.interpolate(unc_ts.unsqueeze(0), (550, 688), mode='bilinear', align_corners=False)[0][0].detach().cpu()
        unc_gen_gmm = unc_ts.numpy()
        ...
        plt.imshow(unc_gen_gmm, cmap='viridis', vmin=unc_base_gmm.min(), vmax=unc_base_gmm.max())
        plt.title('gmm generated')
        plt.axis('off')

        plt.subplot(row_num, column_num, column_num*2 + 2)
        diff = abs(unc_base_gmm-unc_gen_gmm)
        plt.imshow(diff)
        plt.title('gmm abs diff')
        plt.axis('off')

        plt.subplot(row_num, column_num, column_num*2 + 3)
        sum = unc_base_gmm+unc_gen_gmm
        plt.imshow(sum) 
        plt.title('gmm sum')
        plt.axis('off')
